        ...
        <header className='w-full bg-white md:bg-transparent fixed top-0 left-0 right-0'>
            <nav className={`py-4 lg:px-14 px-4 ${isSticky ? "sticky top-0 right-0 border-b bg-white duration-300" : ""}`}>
                <div className='flex justify-between items-center text-base gap-8'>
                    ...
                    {/* <ul className={`md:flex space-x-12 ml-8 ${isMenuOpen ? "flex" : "hidden"} items-center`}> */}
                ...
                <div className={`space-y-4 px-4 mt-16 py-7 bg-brandPrimary list-none ${ isMenuOpen ? "block fixed top-0 right-0 left-0" :"hidden"} `}>
                {navItems.map(({ link, path }) => 
                            <li key={path}>
                                <Link
                                    to={path}
                                    ...
                                    smooth={true}
                                    offset={-100}
                                    className="block text-base  first:font-medium cursor-pointer text-white"
